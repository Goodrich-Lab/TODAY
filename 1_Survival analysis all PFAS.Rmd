...
pfas_survival_models_result<- pfas_survival_models %>%
  ...
  mutate(`PFAS Name` = `pfas_chain`,
         ...
         `P Value` = p.value) %>%
  select(`PFAS Name`, `HR[95%CI]`,`P Value`)

writexl::write_xlsx(pfas_survival_models, 
...
(simplified_pfas_albuminuria <- pfas_survival_models %>%
   dplyr::filter(event == "mic", 
                 !(term %in% c("score_quintile", 
                                 "score_quartile", 
                                 "score_tertile", 
                                 "score_median1")), 
                 str_detect(pfas, "Score", negate = TRUE)) %>% 
   mutate(pfas = fct_reorder(pfas, estimate)) %>%
   ggplot(aes(y = pfas, x = HR)) +
   geom_point(size = 1) +
   geom_errorbar(aes(xmin = exp_conf_low,
                     xmax = exp_conf_high),
                 width = 0) +
   geom_vline(xintercept = 1, linetype = 2) +
   xlab("Diabetic Kidney Disease Risk") +
   xlim(c(0, 10)) +
   theme(strip.text = element_blank(),
         axis.title.y = element_blank(),
         panel.background = element_rect(fill="white"),
...
ggsave(simplified_pfas_albuminuria,
       filename = here::here(dir_figure, "Simplified Survival Coef plot.jpeg"),
       width = 4, height = 2.75) 
