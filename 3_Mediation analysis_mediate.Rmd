...
res_l <- res %>%
  dplyr::select(feature_name, Target,
...
(coef_plot <- res_l %>%
    ...
    ggplot(aes(x = factor(Target,levels = arrange(res, PMed.avg)$Target),
               y = estimate)) +
...
p <- grid.arrange(coef_plot, 
                  ...
                  widths = c(4,2))
#Save
ggsave(p,filename = here::here(dir_figure, "Fig2 Mediation V4.jpeg"),
       width = 8, height = 6)
```
...
med <- read.csv(fs::path(dir_results,'pairwise_mediation_result V1.csv')) %>%
  filter(AptName %in% mim_res$feature_name)
...
med_w <- med %>% 
  dplyr::select(PMed.avg, PMed.avg.lo, PMed.avg.hi,PMed.avg.pval) %>% 
