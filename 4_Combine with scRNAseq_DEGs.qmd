...
full_data_sig_w <- mediation_res_all |> 
  ...
  tidylog::full_join(sc_fdr05_w_pvals, 
                     by = c("EntrezGeneSymbol" = "gene")) |> 
  tidylog::select(-AptName, 
                  -c(var_name:ADE.pval))
...
# write_csv(full_data_sig_w, 
#           fs::path(dir_results,
#                    "Combined_proteomics_scRNAseq_sig_fdr_results_073024.csv"))
...
ggplot(overlapping_sig_l, 
  ...
  geom_line() +  
  geom_point(aes(shape = sig_overall_with_efnumtests, 
                 color = cell_type) ) + 
...
overlap_sig_human_l <- human_overlap |> 
  ...
  pivot_longer(cols = c(estimate_em, conf_low_em, conf_high_em, p_value_em, 
                        estimate_mo, conf_low_mo, conf_high_mo, p_value_mo), 
               names_to = c("variable_human", 
                            "estimate_type_human"), 
               names_pattern = "(.*)_(.*)", 
               values_to = "value_human") 

## b) prep in-vivo data ----
cell_overlap <- overlapping_sig_l |> 
  dplyr::mutate(estimate_type_cell = str_c(cell_type, "; ", week),) |>
  dplyr::rename(p_value = p_val_adj, 
                estimate = avg_log2FC) |>
  tidylog::select(EntrezGeneSymbol, sig_overall, same_dir_em_and_scrna,
                  estimate_type_cell, estimate, p_value) 

overlap_sig_cell_l <- cell_overlap |> 
  pivot_longer(cols = c("estimate", "p_value"), 
               names_to = "variable_cell", values_to =  "value_cell")


## c. Merge datasets and pivot longer on study ----
overlap_combined <- tidylog::full_join(
  overlap_sig_human_l, 
  overlap_sig_cell_l, 
  relationship = "many-to-many")

## Pivot longer then wider
overlap_combined_transformed <- overlap_combined |> 
  pivot_longer(cols = c(value_human, value_cell), 
               names_to = "variable", 
               values_to = "value") |>
  pivot_wider(names_from = "estimate_type_human", 
              values_from = "value_human") |>
  pivot_wider(names_from = "estimate_type_cell", 
              values_from = "value_cell") |>
  dplyr::mutate(study = str_remove_all(variable, "_.*")) |>
  dplyr::select(-variable) |>
  dplyr::mutate(study = factor(study, levels = c("estimate", "conf_low", 
                                                 "conf_high", "p_value"))) 



overlap_combined_long <- overlap_combined |> 
  pivot_longer(
    cols = c(starts_with("variable_"), starts_with("estimate_type_"), starts_with("value_")),
    names_to = c(".value", "study"),
    names_pattern = "(.*)_(.*)") |> 
  ungroup()

overlap_combined_long2 <- overlap_combined_long |>
  pivot_wider(names_from = "variable", values_from = "value") 

# Replace "human" and "cell" in the study column
overlap_combined_long <- overlap_combined_long |> 
  mutate(study = if_else(study == "human", "human", "cell"))



xxx  <- overlap_combined_long |>
  dplyr::summarise(n = dplyr::n(), 
                   .by = c(EntrezGeneSymbol, sig_overall, 
                           same_dir_em_and_scrna, study, 
                           estimate_type, variable)) |>
  dplyr::filter(n > 1L) 
...
overlap_combined_w <- overlap_combined_w 
