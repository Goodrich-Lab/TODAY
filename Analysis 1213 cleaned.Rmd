...
name <- colnames(data %>% dplyr::select(met_names, -var_uni_value$name))
```
...
models <- eo_comb %>%
  mutate(covar = paste0("sex_male+",str_c(covars[-c(2)], collapse = "+")),
...
# fit <- coxph(Surv(yrstohyp2, hyp) ~
#                            strata(score_median) + sex + agebase + serum_creat, data = df)
# 
# fit <- coxph(Surv(yrstoglyc2, glyc) ~
#                            strata(score_median) + sex + agebase + serum_creat, data = df)
# 
# fit <- coxph(Surv(yrstomic2, mic) ~
#                            strata(score_median) + sex + agebase + serum_creat, data = df)
...
data_scale <- data %>% mutate_at(.vars = vars(name),
                                 .funs = ~scale(.)%>%
                                   as.matrix())
## owas
