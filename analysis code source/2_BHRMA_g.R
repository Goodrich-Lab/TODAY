...
# Description:
# input variables
# X: A NxP matrix of exposures for mixture analysis (on the original scale
#    with NA's for individuals with BLD)
# Y: A N-length vector for a continuous outcome
# U: A NxQ matrix of covariates (variables included in the regression 
#    model but not included in the g-estimation)
# LOD: A P-length vector of LODs for each exposure. Individuals with missing
#    data will have data imputed below this level of detection  
# profiles: A 2xP matrix of two counterfactual profiles of exposures for
#    which a potential outcomes risk difference is calculated (as the 
#    exposures are standardized within the function, these profiles should
...
BHRMA.g <- function(X=NULL, Y=NULL, U=NULL, LOD=NULL, profiles=NULL) {
  ...
  for(i in 1:N) {
    ...
    mu[i] <- alpha + 
             inprod(beta[1:P], X.s[i,1:P]) + 
             inprod(delta[1:Q], U[i,1:Q])
  ...
  for(j in 1:P) {
    ...
    gamma[j] ~ dbern(pi)
    for(k in 1:P) {
      T[j,k] <- gamma[j]*gamma[k]*XtX[j,k]/(G) + 
                (1-gamma[j]*gamma[k])*equals(j,k)*pow(prop.sd.beta[j],-2)
    }
  ...
  # Hyper-g prior (following Perrakis 2018, note that this is on 
  # the G^-1 so the Beta distribution is switchd in terms of a and
  # b from Li and Clyde 2019 equation 34)
  a <- 3
  bw <- a/2 - 1
  w~dbeta(1,bw) 
  G <- w/(1-w)
  ...
  # Set N,P,Q,R, and exposure names 
  N <- length(Y)
  P <- ncol(X)
  Q <- ncol(U)
  ...
  model.fit <- jags.model(file=textConnection(ridge.BDL.model), 
...
X.sa =  full_data |> select(pfas_pfos, 
                            pfas_pfhxs, 
                            pfas_nmefosaa) |> 
  mutate(across(everything(), 
                ~log2(.) |> scale() |> as.numeric())) 

X.all =  full_data |> select(all_of(pfas_names_reduced)) |> 
  mutate(across(everything(), 
                ~log2(.) |> scale() |> as.numeric())) 

X.ca =  full_data |> select(pfas_pfoa, 
                            pfas_pfna, 
                            pfas_pfda) |> 
  mutate(across(everything(), 
                ~log2(.) |> scale() |> as.numeric())) 
...
profiles = c(-3,3)*matrix(.5, nrow=2, ncol=P)
...
for(i in length(fit)){
  fit[[i]] <- BHRMA.g(X=X.all, Y=Y, U=U, LOD=LOD, profiles=profiles)
  return(paste0(100*i/length(fit), "% complete"))
}
