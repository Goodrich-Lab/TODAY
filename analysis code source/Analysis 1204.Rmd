...
## Mixture analysis - qgcomp- codi and pfas
```{r mixture analysis codi}
# qgcomp --------
qgcomp <- qgcomp::qgcomp.noboot(
  as.formula(paste0("codi ~", str_c(pfas_names_all, 
                                    collapse = "+"),"+", 
...
### coefficient plot----
coef <- data.frame(qgcomp$fit$coefficients) %>% 
  rownames_to_column("Predictor") %>%
  filter(Predictor %in% pfas_names_all) %>%
  rename(Estimate = qgcomp.fit.coefficients) %>%
...
coef1 <- coef %>% mutate(Predictor = case_when(
  ...
  grepl("pfuna", Predictor) ~ "PFUnA",
  grepl("pfhpa", Predictor) ~"PFHpA",
  grepl("nmefosaa", Predictor) ~"NMeFOSAA",
))

p <- ggplot(coef1,aes(Predictor, Estimate
                      ,
                      fill = direction)) + 
  ...
  geom_text(aes(label = round(Estimate,2)), hjust= -0.05) +
...
## Survival Analysis - PFAS mixture and hyperfiltration
**Significant result were found between mixture and hyperfiltration with u_alb_creat kidney function in the model**

**case1_control0 was excluded from covars**
```{r bootstrap}
set.seed(111)
...
# Save plot
ggsave(out, 
       filename = fs::path(dir_figure, "KM_curve_pfas_mixture_hyp.jpeg"), 
       width = 1.5, 
       height = 1.5, dpi = 600)
```
