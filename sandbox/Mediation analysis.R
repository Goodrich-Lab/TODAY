...
nmed<- length(overlapping_feature)#define number of mediators in your dataset
...
for(name in omic_names){
  ...
  #   data = d2,
  #   med.model= lm(mediator~pfas_pfna+sex_male+agebase+serum_creat+dxtime,
  #                                              data=d2),
  #   out.model = coxph(Surv(daystomic,mic)~pfas_pfna+mediator+sex_male+agebase+serum_creat+dxtime,
  #                                           data=d2),
  #   treat="pfas_pfna")
  # 
  # med<- mediation::mediate(
  #   data = d2,
  #   model.m= lm(mediator~pfas_pfna+sex_male+agebase+serum_creat+dxtime,
  #                 data=d2),
  #   model.y = survreg(Surv(daystomic,mic)~pfas_pfna+mediator+sex_male+agebase+serum_creat+dxtime,
  #                     data=d2),
  #   treat="pfas_pfna",
  #   mediator = "mediator")
  ...
  med.results[k,]<-cbind(nobs(y),med$d0, med$d0.ci[1], med$d0.ci[2], med$d0.p, med$d1, med$d1.ci[1],
        ...
        rownames(med.results)[k]<-paste(colnames(d2)[1],colnames(d2)[2],colnames(d2)[2+ncovars+1],sep='_')  
        print(k) 
...
write_csv(med_result,fs::path(dir_result,"mediation", 'pairwise_mediation_ikids.csv'))
